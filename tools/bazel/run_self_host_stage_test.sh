...
grep -q '@.stage3.tinystdencoding.ir' "$strict_stage3_ir_dir/merged.ll"
...
if [ -n "$LLC_BIN" ]; then
  ...
  if [ "$strict_stage3_std_encoding_output" != "$strict_stage3_std_encoding_expected" ]; then
    printf 'Expected strict stage3 generated compiler std encoding output mismatch\n' >&2
    printf 'Expected:\n%s\n' "$strict_stage3_std_encoding_expected" >&2
    printf 'Got:\n%s\n' "$strict_stage3_std_encoding_output" >&2
    exit 1
  fi
